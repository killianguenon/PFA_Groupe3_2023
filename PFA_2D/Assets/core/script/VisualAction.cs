...
public class VisualAction : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        function.ScroolAndScale2D(gameObject, targetButtonAction, vector2DistanceStart); // utilise cette fonction pour que l'objet adapte son scale à celui de sa cible 

        this.GetComponent<Rigidbody2D>().AddRelativeForce(vector2Strength); // aplique le vecteur de force pour son déplacement
    }
